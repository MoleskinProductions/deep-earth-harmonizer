...
def test_invalid_bbox():
    ...
    with pytest.raises(ValueError, match="Invalid latitude"):
        CoordinateManager(lat_min=-95, lat_max=45, lon_min=-93, lon_max=-92)
    
    # Longitude out of range
    with pytest.raises(ValueError, match="Invalid longitude"):
        CoordinateManager(lat_min=44, lat_max=45, lon_min=-190, lon_max=-92)
    
    # Min > Max
    with pytest.raises(ValueError, match="must be less than"):
        CoordinateManager(lat_min=45, lat_max=44, lon_min=-93, lon_max=-92)

def test_utm_zone_detection():
    # Minneapolis (~45N, -93W) should be UTM 15N
    cm = CoordinateManager(lat_min=44.9, lat_max=45.1, lon_min=-93.1, lon_max=-92.9)
    assert cm.utm_zone == "15N"
    assert cm.utm_epsg == 32615

    # London (~51N, 0W) should be UTM 30N or 31N depending on exact lon
    cm = CoordinateManager(lat_min=51.4, lat_max=51.6, lon_min=-0.1, lon_max=0.1)
...
def test_get_utm_bbox():
    ...
    assert utm_bbox[0] < utm_bbox[2]
    assert utm_bbox[1] < utm_bbox[3]